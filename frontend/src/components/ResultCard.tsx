...
export default function ResultCard({ result, index, onToast }: ResultCardProps) {
  ...
  const hasExplanation =
    result.explanation &&
    (result.explanation.yt_music_explanation?.title ||
      result.explanation.yt_playlist_explanation?.title);
    ...
    >
        ...
        <div className="relative flex items-center justify-center">
          <svg width="64" height="64" className="-rotate-90">
            ...
            <circle
              ...
              strokeDashoffset={2 * Math.PI * 26 - (result.seo_score / 100) * 2 * Math.PI * 26}
      ...
      <div className="mb-3">
        ...
        <div className="group flex items-start justify-between gap-2 rounded-lg bg-zinc-800/70 p-3">
          <p className="text-sm leading-relaxed text-zinc-200">{result.yt_music_title}</p>
          <button
      ...
      <div className="mb-4">
        ...
        <div className="group flex items-start justify-between gap-2 rounded-lg bg-zinc-800/70 p-3">
          <p className="text-sm leading-relaxed text-zinc-200">{result.yt_playlist_title}</p>
          <button
            onClick={() => copyToClipboard(result.yt_playlist_title, `playlist-${index}`)}
            className="shrink-0 rounded p-1 text-zinc-500 hover:bg-zinc-700 hover:text-zinc-300 transition-colors"
            title="복사"
          >
        ...
        <div className="mb-4">
          ...
          {showExplanation && result.explanation && (
            <div className="mt-3 space-y-4 rounded-lg border border-zinc-800/80 bg-zinc-950/60 p-4 animate-fade-in">
              <ExplanationPanel
                explanation={result.explanation.yt_music_explanation}
                label="YT Music 제목"
              />
              <div className="border-t border-zinc-800/50" />
              <ExplanationPanel
                explanation={result.explanation.yt_playlist_explanation}
      ...
      <div className="mt-auto rounded-lg border border-zinc-800 bg-zinc-950/50 p-4">
        ...
        <div className="space-y-2.5 text-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-zinc-500">인물 포함: {result.thumbnail.has_person ? "예" : "아니오"}</span>
          </div>
        </div>
      </div>
    </div>
